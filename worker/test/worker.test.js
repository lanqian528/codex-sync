...
test('unauthenticated, wrong and absent keys never read or write storage', async () => {
  ...
  assert.equal((await worker.fetch(req('/api/admin', 'PUT', config, { Authorization: 'Bearer wrong', 'If-Match': 'initial' }), env)).status, 401);
...
test('same access key reads, edits and survives a new handler invocation', async () => {
  const { env } = fixture();
  const saved = await worker.fetch(req('/api/admin', 'PUT', config, { 'If-Match': 'initial' }), env);
...
test('Pro preserves key, replacement updates it, stale writes fail', async () => {
  const { env, saved } = fixture();
  let res = await worker.fetch(req('/api/admin', 'PUT', config, { 'If-Match': 'initial' }), env);
  const etag = res.headers.get('etag');
  res = await worker.fetch(req('/api/admin', 'PUT', { mode: 'pro', base_url: 'https://new.example/v1' }, { 'If-Match': etag }), env);
  assert.equal(res.status, 200); assert.equal(saved().config.api_key, config.api_key);
  assert.equal((await worker.fetch(req('/api/admin', 'PUT', config, { 'If-Match': etag }), env)).status, 409);
  res = await worker.fetch(req('/api/admin', 'PUT', { ...config, api_key: 'new-fake-key' }, { 'If-Match': res.headers.get('etag') }), env);
...
test('invalid input, unsupported method and missing precondition do not save', async () => {
  ...
  for (const data of [{...config, mode:'bad'}, {...config, base_url:'http://bad.test'}, {...config, api_key:''}, {...config, extra:1}, {...config, api_key:'secret\nheader'}]) {
    assert.equal((await worker.fetch(req('/api/admin','PUT',data,{'If-Match':'initial'}),env)).status,400);
  }
...
test('login issues secure cookie; CSRF, logout and key rotation', async () => {
  ...
  const auth={Authorization:'',Cookie:cookie,Origin:'https://example.com','X-CSRF-Token':csrf,'If-Match':'initial'};
...
test('unconfigured storage is read-only, API key stays out of admin responses',async()=>{
  ...
  assert.equal((await worker.fetch(req('/api/admin','PUT',config,{'If-Match':'initial'}),env)).status,503);
});
